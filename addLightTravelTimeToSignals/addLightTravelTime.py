...
raDecDirectory = "C://Users//openspace//Agnes//convertToJSON//addLightTravelTimeToSignals//radecData"
pathlistRadec = Path(raDecDirectory).glob('**/*.dat')

for path in pathlistRadec:
	raDecPathStr = str(path)

	with open(raDecPathStr) as data_file:    
		spaceCraft = raDecPathStr[77:81];
		# print(spaceCraft)
		...
		for path in pathlist:
				...
				with open(str('radecData/' + spaceCraft + '.dat')) as f:
						...
						with open(path_in_str) as data_file:    
							...
							for i in range(0, len(signal)):
								if(signal[i]["bot"][0:8] in row['TimeStamp'][0:8]): 
									print(spaceCraft)
									if( signal[i]["projuser"] == spaceCraft):
		...
		for j in range(0, len(testSignal)):
				if (j+1 < len(testSignal) and (testSignal[j]["bot"][0:8] != testSignal[j+1]["bot"][0:8])):
					filename = str("signalData/" + testSignal[j]["bot"][0:9]+'.json' )
